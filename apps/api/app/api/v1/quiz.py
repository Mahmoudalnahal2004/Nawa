...
from app.schemas.quiz import QuizStartRequest, QuizSessionResponse, AnswerRequest, AnswerFeedback, QuizResultSummary
...
async def start_quiz(data: QuizStartRequest, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)):
    try:
        return await quiz_service.start_quiz(db, user.id, data.category_id, data.num_questions)
...
async def get_quiz(session_id: str, user: User = Depends(get_current_active_user)):
    ...
    return QuizSessionResponse(session_id=session_id, questions=session["questions"], total_questions=len(session["questions"]), category_name=session["category_name"])
